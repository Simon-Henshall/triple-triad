...
export const utils = {
  ...
  pickPlayerCards(ownedCardsJSON) {
    ...
    UIManager.selectionBoard.displayedCard = null;
    ...
    for (var i = 0; i < parsedCards.length; i++) {
      if (parsedCards[i].count > 0) {
        UIManager.cardCount = parsedCards[i].count;
        if (cardsCopy[i]) {
    ...
    if (Game.rules.indexOf("random") != -1) {
      ...
      UIManager.selectionBoard.container.addChild(UIManager.selectionBoard.background);
      ...
      UIManager.selectionBoard.pageDisplay.x = UIManager.selectionBoard.background.x + 150;
      UIManager.selectionBoard.pageDisplay.y = UIManager.selectionBoard.background.y + 20;
      UIManager.selectionBoard.pageDisplay.textBaseline = "alphabetic";

      var numText = new createjs.Text("NUM.", "20px Arial", "#ffffff");
      numText.x = UIManager.selectionBoard.background.x + 350;
      numText.y = UIManager.selectionBoard.background.y + 20;
